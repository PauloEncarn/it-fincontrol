...
export default function FileDrop({ label, onFileSelect, existingFile, metaData, addToast }) {
  ...
  return (
    <Box>
      ...
      <Tooltip title={displayFileName || 'Selecionar arquivo'}>
        <Box
          component="label"
          sx={{
            position: 'relative',
        ...
        >
          ...
          <Stack alignItems="center" spacing={0.5} sx={{ minWidth: 0, width: '100%' }}>
            {displayFileName ? <CheckCircleOutlinedIcon fontSize="small" /> : <CloudUploadOutlinedIcon fontSize="small" />}
            <Typography
              variant="caption"
              fontWeight={800}
              sx={{
