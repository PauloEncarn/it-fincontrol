...
export default function DashboardView({ notas, solicitacoes, filiais }) {
  ...
  const pendentes = dadosNotas.lista.filter((nota) => nota.status_pagamento === 'Pendente Lançamento').length;
  const concluidas = dadosNotas.lista.filter((nota) => nota.status_pagamento === 'Concluída').length;
  const emProcesso = dadosNotas.lista.filter((nota) => nota.status_pagamento?.includes('Aguardando')).length;
