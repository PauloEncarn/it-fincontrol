...
import { 
  ChevronDown, Plus, Server, Filter, 
  Calendar, Clock, AlertTriangle, CheckCircle, X, 
  UploadCloud, FileText, Copy, Edit2, Paperclip, ClipboardList
} from 'lucide-react';

const API_URL = "http://127.0.0.1:8000";

const LABEL_STYLE = "text-xs font-bold uppercase text-slate-500 mb-1.5 block tracking-wide";
const INPUT_STYLE = "w-full border-2 border-slate-200 p-3 rounded-xl text-sm font-medium text-slate-700 focus:border-indigo-500 outline-none transition-all bg-white";
...
const FileDrop = ({ label, onFileSelect, existingFile, metaData }) => {
  ...
  useEffect(() => { if(!existingFile) setFileName(null); }, [existingFile]);

  const handleFile = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (!metaData.fornecedor || !metaData.nota) return alert("Selecione Fornecedor e Nota primeiro.");
    setFileName(file.name);
    const fd = new FormData();
    fd.append("file", file); fd.append("fornecedor", metaData.fornecedor);
    fd.append("nota", metaData.nota); fd.append("vencimento", metaData.vencimento || "S_D");
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative border-2 border-dashed border-indigo-300 rounded-xl p-4 hover:bg-indigo-50 text-center cursor-pointer group bg-white">
        ...
        <div className="flex flex-col items-center justify-center gap-2">
          {fileName ? <FileText className="text-indigo-600" size={24}/> : <UploadCloud className="text-indigo-400 group-hover:text-indigo-600" size={24}/>}
          <span className="text-sm font-medium text-slate-600 truncate max-w-full px-2">{fileName || "Anexar Arquivo"}</span>
        </div>
      </div>
    </div>
  );
...
export default function ITFinControl() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const hoje = new Date();
      setExpandedMonth(`${String(hoje.getMonth() + 1).padStart(2, '0')}/${hoje.getFullYear()}`);
  ...
  const handleFornecedorChange = (id) => {
    ...
    if (forn) {
      ...
      // Puxa descrição padrão do cadastro
      setForm({ 
          ...form, 
  ...
  const copiarParaProtheus = (nota) => {
    // Formato: FORNECEDOR: DESCRIÇÃO | CPF/CNPJ: ... | NF: ... | Valor R$: ... | Vencimento: ...
    const venc = nota.data_vencimento.split('-').reverse().join('/');
    const valor = nota.valor.toLocaleString('pt-BR', {minimumFractionDigits: 2});
    
    // Tenta pegar descrição da nota, se não tiver, pega do fornecedor (que já salvamos na nota no backend)
  ...
  const getGroupedData = () => {
    ...
    return Object.entries(grupos).sort().reverse();
  };

  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
      <header className="bg-white px-8 py-5 sticky top-0 z-20 flex justify-between items-center shadow-sm border-b border-slate-200">
        <div className="flex items-center gap-4">
           <div className="bg-indigo-600 p-3 rounded-xl text-white shadow-lg shadow-indigo-200"><Server size={24} /></div>
        ...
        <div className="flex gap-4">
           <select className="bg-slate-50 border-2 border-slate-200 p-2.5 rounded-xl text-sm font-bold text-slate-700 hover:border-indigo-400 transition-colors outline-none cursor-pointer" 
           ...
           <button onClick={() => { setForm(initialForm); setIsEditMode(false); setShowModal(true); }} 
             className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm flex gap-2 items-center shadow-lg shadow-indigo-200 transition-all active:scale-95">
             <Plus size={20}/> Novo
           </button>
        </div>
      </header>
          ...
          {getGroupedData().map(([mes, fornecedoresDict]) => (
            <div key={mes} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
               <div onClick={() => setExpandedMonth(expandedMonth === mes ? null : mes)} 
                 className="bg-gradient-to-r from-slate-50 to-white p-5 border-b border-slate-200 flex justify-between cursor-pointer hover:from-indigo-50 transition-all">
                       ...
                       <div key={nomeForn} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                         ...
                         {isExpanded && (
                           <div className="divide-y divide-slate-100">
                             {notas.map(nota => (
                               <div key={nota.id} className="p-6 hover:bg-slate-50 transition-colors group">
                                 <div className="flex flex-col xl:flex-row justify-between gap-6 mb-5">
                                    <div className="flex items-start gap-5">
                                      <div className={`w-1.5 h-12 rounded-full ${STATUS_COLORS[nota.status_pagamento].split(' ')[0].replace('bg-', 'bg-').replace('-50', '-500')}`}></div>
                                      <div>
                                        <div className="flex items-center gap-3">
                                            <span className="text-xl font-black text-slate-800">NF {nota.numero_nota}</span>
                                            <span className="text-xs font-bold text-slate-500 border border-slate-300 px-2 py-0.5 rounded uppercase bg-slate-100">{nota.serie}</span>
                                        </div>
                                        <div className="flex gap-4 mt-1.5 text-xs text-slate-500 font-bold uppercase tracking-wide">
                                    ...
                                    <div className="flex items-center gap-6">
                                       ...
                                       <div className="flex gap-2">
                                         <button onClick={() => copiarParaProtheus(nota)} className="p-2.5 bg-green-50 text-green-700 rounded-lg border border-green-100 hover:bg-green-100 font-bold text-xs flex flex-col items-center justify-center w-16 transition-all" title="Copiar p/ Protheus"><ClipboardList size={18}/>Copiar</button>
                                         {nota.arquivo_nota && <button onClick={() => window.open(`${API_URL}/${nota.arquivo_nota}`, '_blank')} className="p-2 bg-red-50 text-red-600 rounded-lg border border-red-100 hover:bg-red-100 font-bold text-xs flex flex-col items-center justify-center w-16" title="Baixar Nota"><FileText size={16}/>Nota</button>}
                                         {nota.arquivo_boleto && <button onClick={() => window.open(`${API_URL}/${nota.arquivo_boleto}`, '_blank')} className="p-2 bg-blue-50 text-blue-600 rounded-lg border border-blue-100 hover:bg-blue-100 font-bold text-xs flex flex-col items-center justify-center w-16" title="Baixar Boleto"><Paperclip size={16}/>Boleto</button>}
                                         <button onClick={() => abrirEdicao(nota)} className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg border border-transparent hover:border-indigo-100 transition-all"><Edit2 size={18}/></button>
                                         <button onClick={() => { setIsEditMode(false); setForm({...nota, id:null, numero_nota:'', arquivo_nota:'', arquivo_boleto:''}); setShowModal(true); }} className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg border border-transparent hover:border-indigo-100 transition-all"><Copy size={18}/></button>
                                       </div>
                                    </div>
                                 </div>
                                 
                                 <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-y-4 gap-x-6 text-sm text-slate-600 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                                   <div><strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Vencimento</strong>{nota.data_vencimento.split('-').reverse().join('/')}</div>
                                   <div><strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Emissão</strong>{nota.data_emissao.split('-').reverse().join('/')}</div>
                                   <div><strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Centro Custo</strong>{nota.centro_custo_usado}</div>
                                   <div><strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Medição / Pedido</strong>{nota.numero_medicao || '-'} / {nota.numero_pedido || '-'}</div>
                                   <div><strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Fluig</strong>{nota.solicitacao_fluig || '-'}</div>
                                   <div className="xl:col-span-1">
                                     <strong className="block text-indigo-400 uppercase text-[10px] font-bold mb-1">Status</strong>
                                     ...
                                     }} className={`w-full text-xs font-bold py-1.5 px-2 rounded-lg border cursor-pointer outline-none ${STATUS_COLORS[nota.status_pagamento]}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl p-8 my-10 relative border border-slate-100 animate-in fade-in zoom-in duration-200">
             <button onClick={() => setShowModal(false)} className="absolute top-6 right-6 text-slate-300 hover:text-red-500 transition-colors"><X size={32}/></button>
             <h2 className="text-3xl font-black mb-8 flex gap-3 text-slate-800 items-center">
                <div className="bg-indigo-100 p-3 rounded-xl text-indigo-600"><Plus size={28}/></div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div><label className={LABEL_STYLE}>Status Inicial</label>
                   <select className={INPUT_STYLE} value={form.status_pagamento} onChange={e => setForm({...form, status_pagamento: e.target.value})}>
                      {OPCOES_STATUS.map(s => <option key={s} value={s}>{s}</option>)}
                   </select>
                </div>

                <div><label className={LABEL_STYLE}>Emissão *</label><input type="date" className={INPUT_STYLE} value={form.data_emissao} onChange={e => setForm({...form, data_emissao: e.target.value})}/></div>
                ...
                <div className="col-span-2"><label className={LABEL_STYLE}>Recebimento</label><input type="date" className={INPUT_STYLE} value={form.data_recebimento || ''} onChange={e => setForm({...form, data_recebimento: e.target.value})}/></div>

                <div className="col-span-4 bg-indigo-50/50 p-5 rounded-xl border border-indigo-100 grid grid-cols-3 gap-5 mt-2">
                    <div className="col-span-3 text-xs font-black text-indigo-400 uppercase tracking-widest">Controle T.I.</div>
                    <div><label className={LABEL_STYLE}>1. Medição</label><input className={INPUT_STYLE} value={form.numero_medicao} onChange={e => setForm({...form, numero_medicao: e.target.value})}/></div>
                    <div><label className={LABEL_STYLE}>2. Pedido</label><input className={INPUT_STYLE} value={form.numero_pedido} onChange={e => setForm({...form, numero_pedido: e.target.value})}/></div>
                    <div><label className={LABEL_STYLE}>3. Fluig</label><input className={INPUT_STYLE} value={form.solicitacao_fluig} onChange={e => setForm({...form, solicitacao_fluig: e.target.value})}/></div>
                ...
                <div className="col-span-4 mt-2">
                  ...
                  <textarea className={`${INPUT_STYLE} h-24 resize-none`} value={form.observacao} onChange={e => setForm({...form, observacao: e.target.value})}/>
                </div>
             </div>

             <button onClick={salvarForm} className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold py-4 rounded-xl mt-8 text-lg shadow-xl shadow-indigo-200 transition-all active:scale-[0.98]">
